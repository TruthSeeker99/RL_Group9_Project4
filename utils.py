...
def preprocess(obs, width=192, height=112, gray=True):
    ...
    for i in range(0, fs, 3):
        ...
        if gray:
            ...
            new_frame = []
            for j in range(3):
                new_frame.append(cv2.resize(frame[j], (width, height), interpolation=cv2.INTER_LINEAR))
            frame = np.stack(new_frame, axis=-1)
        # print(frame.shape)
        result.append(frame)
    ...
    if gray:
        ...
        frames = np.concatenate(result, axis=-1)
...
def record_single_video(env, agent, video_folder, video_length=10240, env_id=None, all_settings=None, episodes=1):
    ...
    gray = all_settings['basic']['frame_shape'][2]
    episode_count = 0
    count = 0
    # selected_actions = [[9, 0], [0, 0], [0, 0]]

    while episode_count < episodes:
        preprocess(observation, width=width, height=height, gray=gray)
