...
def main():
    ...
    # Create environment
    model_folder = './ckpts/'
    model_checkpoint = 'dqn_sfiii3n'
    env, num_envs = make_sb3_env("sfiii3n", settings, wrappers_settings)
    print("Activated {} environment(s)".format(num_envs))
    auto_save_callback = AutoSave(check_freq=10000, num_envs=num_envs,
                                  save_path=model_folder, filename_prefix=model_checkpoint + "_")
    model_path = os.path.join(model_folder, model_checkpoint)


    # Instantiate the agent
    agent = DQN("MultiInputPolicy", env, verbose=1, buffer_size=50000, batch_size=32, tensorboard_log=f"./logs/")
    # Train the agent
    agent.learn(
